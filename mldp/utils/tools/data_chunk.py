...
import numpy as np
from mldp.utils.helpers.validation import equal_vals
from mldp.utils.errors import DataChunkError
from .data_unit import DataUnit
from mldp.utils.tools.dc_writers import JsonWriter, CsvWriter

# TODO: think if it make sense to perform assignment by copying the values
# TODO: instead of assignment by reference.
...
class DataChunk(object):
    ...
    def append(self, data_unit_dict):
        """
        Appends a new data-unit to the end of a valid data-chunk.

        :param data_unit_dict: dict with field name and value pairs.
        """
        allowed_types = (dict, OrderedDict, DataUnit)
        rpr = [at.__name__ for at in allowed_types]
        if not isinstance(data_unit_dict, allowed_types):
            raise TypeError("'data_unit' must be %s." % " or ".join(rpr))
        if not self._is_valid():
            raise DataChunkError("Can't append a new data-unit to an "
                                 "invalid data-chunk.")

        for k in data_unit_dict:
            if k not in self:
                raise ValueError("Please provide all keys matching existing "
                                 "field names.")
        for k in data_unit_dict:
            self[k] = np.append(self[k], data_unit_dict[k])
    ...
    def __delitem__(self, key):
        ...
        If data-chunk is invalid, it perform deletion of data-units without
        a guarantee of execution and effect.
        """
