from mldp.utils.constants.dp import TERMINATION_TOKEN
from mldp.steps.readers import BaseReader
from mldp.steps.readers.helpers import create_openers_of_valid_files,\
    populate_queue_with_chunks
from pandas.io.parsers import TextFileReader
from mldp.utils.tools import DataChunk
from mldp.utils.helpers.validation import validate_data_paths
...
class CsvReader(BaseReader):
    ...
    def _iter(self, data_path):
        ...
        file_openers = create_openers_of_valid_files(data_paths, ext='csv',
                                                     encoding=self.parser_kwargs['encoding'])
        if not file_openers:
            raise ValueError(
    ...
    def get_data_chunk_iter(file_opener, **parser_kwargs):
        ...
        f = file_opener()
        try:
            pandas_iter = _TextParser(f, iterable=True, **parser_kwargs)
        except Exception as e:
            raise e
